...
extern "C" {
    fn getfsstat64(buf: *mut libc::statfs, bufsize: libc::c_int, flags: libc::c_int) -> libc::c_int;
}
...
pub fn mounts() -> Result<Vec<libc::statfs>> {
    let expected_len = unsafe {
        getfsstat64(ptr::null_mut(), 0, MNT_NOWAIT)
    };
    ...
    if result == -1 {
        ...
        debug_assert!(expected_len == result,
            "Expected {} statfs entries, but got {}", expected_len, result);
        unsafe {
