...
//!
//! This is a straight-forward copy of the `tokio` blocking pool
//! with the following changes:
//!
//!  1. tokio `oneshot` channel was replaced by the futures `oneshot`
//!  2. processes name prefix changed to `heim-` in order not to make any confusion
//!     why there are some `tokio`-named threads when no `tokio` dependency used.
//!
//! This file originally copyrighted to the "Tokio Contributors",
//! published under the MIT lisense (https://github.com/tokio-rs/tokio/blob/master/LICENSE),
//! and the original source is here: https://github.com/tokio-rs/tokio/blob/2a181320b708f7d03dfb00502b1d6cc783ce0927/tokio-executor/src/blocking.rs
...
fn spawn_thread() {
    let _ = thread::Builder::new()
        .name("heim-blocking-driver".to_string())
        .spawn(|| {
            let mut shared = POOL.shared.lock().unwrap();
            loop {
                // BUSY
