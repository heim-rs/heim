...
#[allow(trivial_casts)]
pub fn host_vm_info() -> Result2<vm_statistics64> {
    ...
    if result != kern_return::KERN_SUCCESS {
        Err(Error2::last_os_error().with_ffi("host_statistics64"))
    } else {
        Ok(stats)
    }
}
